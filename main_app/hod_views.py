...
def add_course(request):
    ...
    context = {"form": form, "page_title": "Add Faculty / Department"}
    if request.method == "POST":
        if form.is_valid():
...
def manage_staff(request):
    allStaff = CustomUser.objects.filter(user_type=2).select_related(
        "staff", "staff__course"
    )
...
def manage_course(request):
    ...
    context = {"courses": courses, "page_title": "Manage Faculties / Departments"}
